...
class UpdateOffer extends StatefulWidget {
  final id;
  UpdateOffer({Key key, this.id}) : super(key: key);
...
class _UpdateOfferState extends State<UpdateOffer> {
  ...
  Future getOffer() async {
    ...
    var qn = await firestore.collection('offers').doc(widget.id).get();
    setState(() {
      fileSelected = true;
      nameController.text = qn.get('companyName');
      descriptionController.text = qn.get('offerDescription');
      offerController.text = qn.get('offer');
      botPointsController.text = qn.get('botPoints').toString();
      logoUrl = qn.get('logo');
    });
  }
